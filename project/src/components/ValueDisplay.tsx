...
const ValueDisplay: React.FC<ValueDisplayProps> = ({ valueRange, formData, onReset }) => {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: 'My Golf Cart Calculation', // Changed from Valuation
        text: shareText,
  ...
  return (
    <div className="lg:w-1/2 bg-gradient-to-br from-[#2D5937]/10 to-[#D4AF37]/10 p-6 lg:p-8 animate-fadeIn">
      <h3 className="text-2xl font-bold text-gray-800 mb-6">Your Cart Calculation</h3> {/* Changed from Valuation */}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button 
          onClick={handleShare}
          className="flex-1 flex items-center justify-center py-2 px-4 bg-[#D4AF37] text-white rounded-md hover:bg-[#c9a533] transition-colors"
        >
          {copied ? 'Copied!' : (
            <>
        ...
        <button 
          onClick={onReset}
          className="flex-1 flex items-center justify-center py-2 px-4 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors"
        >
